...
import { 
  ...
  Loader2, Info, CheckCircle2, AlertCircle, ArrowDownCircle
} from 'lucide-react';
...
const FinancialAgenda = ({ title, description }: FinancialAgendaProps) => {
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          <h2 className="text-3xl font-black text-apple-black flex items-center gap-3">
            <CalendarIcon className="text-orange-500" size={32} /> {title || 'Agenda Financeira'}
          </h2>
          <p className="text-apple-muted mt-1 font-medium">{description || 'Previsão diária de fluxo de caixa e compromissos.'}</p>
        </div>
