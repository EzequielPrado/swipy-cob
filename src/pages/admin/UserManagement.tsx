...
const UserManagement = () => {
  ...
  const handleUpdateToken = async () => {
    ...
    if (error) showError(error.message);
      ...
      showSuccess("Token Woovi atualizado com sucesso!");
    ...
    <AppLayout>
      <div className="flex flex-col gap-8">
        <div>
          ...
          <p className="text-zinc-400 mt-1">Aprove contas, suspenda acessos e configure tokens Woovi.</p>
        </div>
            ...
            <table className="w-full text-left">
              <thead className="bg-zinc-950/50 text-zinc-400 text-xs uppercase tracking-wider border-b border-zinc-800">
                <tr>
                  ...
                  <th className="px-6 py-5">Token Woovi</th>
                  ...
                  <tr key={u.id} className="hover:bg-zinc-800/30 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-semibold text-zinc-100">{u.company || 'Pessoa Física'}</p>
                    ...
                    <td className="px-6 py-4">
                      {u.is_admin ? <ShieldCheck className="text-orange-500" size={18} /> : <span className="text-zinc-700 text-xs">Não</span>}
                    </td>
                    <td className="px-6 py-4">
                      {u.woovi_api_key ? (
      ...
      <Dialog open={isTokenModalOpen} onOpenChange={setIsTokenModalOpen}>
        <DialogContent className="bg-zinc-900 border-zinc-800 text-zinc-100">
          <DialogHeader>
            <DialogTitle>Configurar Token Woovi</DialogTitle>
          </DialogHeader>
          <div className="py-4 space-y-4">
            <div className="p-3 bg-orange-500/5 border border-orange-500/10 rounded-lg">
              <p className="text-[10px] text-orange-500 font-bold uppercase mb-1">Usuário</p>
