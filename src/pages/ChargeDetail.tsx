...
const ChargeDetail = () => {
  ...
  const handleMarkAsPaid = async () => {
    ...
    try {
      ...
      // 3. SE HOUVER PEDIDO VINCULADO: Inteligência Operacional
      if (charge.quote_id) {
        // Buscar itens para verificar se há produção
        ...
        // Gerar ordens industriais se necessário
      ...
      await supabase.from('notification_logs').insert({ 
        ...
        type: 'payment', 
        status: 'success', 
        message: 'Fatura PAGA. Fluxo industrial/operacional acionado.' 
      });

      showSuccess("Pagamento confirmado e pedido liberado!"); 
  ...
  const handleResendWhatsApp = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://mxkorxmazthagjaqwrfk.supabase.co/functions/v1/resend-charge-whatsapp`, {
        ...
        body: JSON.stringify({ chargeId: id, origin: window.location.origin })
      });
      const result = await response.json();
      if (!response.ok) throw new Error(result.error);
      showSuccess("Cobrança reenviada!");
      fetchDetails(); 
  ...
  return (
    <AppLayout>
          ...
          <div className="space-y-8">
             <div className="bg-apple-white border border-apple-border rounded-[2rem] p-8 shadow-sm">
                ...
                <div className="mt-8 pt-8 border-t border-apple-border space-y-3">
                   {charge.status === 'pago' ? (
                      <button onClick={() => setIsFiscalModalOpen(true)} className="w-full py-5 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all bg-blue-600 text-white hover:bg-blue-700 flex items-center justify-center gap-2 shadow-xl shadow-blue-600/10 active:scale-95"><ReceiptText size={18} /> EMITIR NOTA FISCAL</button>
                   ) : (
                     <>
                        {charge.method !== 'manual' && (<button onClick={handleResendWhatsApp} disabled={!!actionLoading} className="w-full py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all active:scale-95 shadow-sm bg-apple-offWhite text-apple-dark border border-apple-border hover:bg-apple-light flex items-center justify-center gap-2">{actionLoading === 'resend' ? <Loader2 className="animate-spin" size={16} /> : <Send size={16} />} REENVIAR WHATSAPP</button>)}
                        <button onClick={() => setIsPayModalOpen(true)} className="w-full py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all active:scale-95 shadow-xl bg-apple-black text-white hover:bg-zinc-800">BAIXAR MANUALMENTE</button>
                     </>
                   )}
                </div>
             </div>
      ...
      <IssueInvoiceModal isOpen={isFiscalModalOpen} onClose={() => setIsFiscalModalOpen(false)} onSuccess={fetchDetails} defaultData={charge ? { customerId: charge.customer_id, amount: charge.amount.toString(), description: charge.description || `Referente à cobrança #${charge.id.split('-')[0].toUpperCase()}` } : undefined} />
    </AppLayout>
  );
};
