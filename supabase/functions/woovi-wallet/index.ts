...
serve(async (req) => {
  ...
  try {
    ...
    // Pegamos o ID do usuário do cabeçalho de autorização
    const authHeader = req.headers.get('Authorization')!
    const token = authHeader.replace('Bearer ', '')
    const { data: { user } } = await supabaseClient.auth.getUser(token)
    if (!user) throw new Error("Não autorizado")

    // Busca a API Key do usuário
    ...
    if (!profile?.woovi_api_key) throw new Error("Configure sua API Key da Woovi nas configurações.")

    const url = new URL(req.url)
    const action = url.searchParams.get('action') || 'balance'

    if (action === 'balance') {
      // Consulta saldo na Woovi
      // Nota: O endpoint real da Woovi para saldo pode variar conforme a versão da API deles
    ...
    if (action === 'withdraw') {
      ...
      // Solicita saque (Cashout) na Woovi
